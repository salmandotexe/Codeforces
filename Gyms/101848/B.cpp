...
int main () {
    ...
    if(n<=4) {
        if(n==3) {
           printf("%lld %lld %lld\n", ara[0], ara[0]+1, ara[0]+2);
        }
        else if(n==4) {
            printf("%lld %lld %lld %lld\n", ara[0], ara[0]+1, ara[0]+2, ara[0]+3);
        }
        return 0;
    }
    ...
    for(int i=0; i<5; i++) {
        for(int j=i+1; j<5; j++) {
            ...
            LL diff = (ara[indx2] - ara[indx1]);


            LL terms = j - i;

            if(diff%terms!=0) continue;

            LL comDiff = diff/terms;

            LL firstElem = ara[indx1] - (indx1*diff);
            ...
            if(ara[0]!=tempAra[0]) cnt++;

            for(int k=1; k<n; k++) {
                tempAra[k] = tempAra[k-1] + comDiff;

                if(ara[k]!=tempAra[k]) cnt++;
                if(cnt>3) {
                    flag = 0;
                    break;
                }
