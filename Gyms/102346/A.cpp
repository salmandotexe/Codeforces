...
LL R, C, K;
LL r[SIZE], c[SIZE], s[SIZE];
LL minR[SIZE], maxR[SIZE], minC[SIZE], maxC[SIZE];
...
void Union(int a, int b) {
    ...
    if(u!=v) {
        ...
        minR[v] = min(minR[v], minR[u]);
        minC[v] = min(minC[v], minC[u]);
        maxR[v] = max(maxR[v], maxR[u]);
        maxC[v] = max(maxC[v], maxC[u]);
    }
}
...
int main () {
    ...
    for(int i=0; i<K; i++) {
        ...
        if( (minR[curr]<=0 && maxR[curr]>=R) || (minC[curr]<=0 && maxC[curr]>=C) || (maxR[curr]>=R && maxC[curr]>=C) || (minR[curr]<=0 && minC[curr]<=0)) {
            cout << "N\n";
            return 0;
        }
    }
...
//        cout << i << " = " << parent[i] << "\n";
//
//        cout << "mnR = " << minR[parent[i]] << ", mxR = " << maxR[parent[i]] << endl;
//        cout << "mnC = " << minC[parent[i]] << ", mxC = " <<  maxC[parent[i]] << endl;
//
//    }
