...
int main () {
    ...
    for(int ks=1; ks<=tc; ks++) {
        ...
        for(int i=1; i<=n; i++) {
            scanf("%d", &ara[i]);
            if(ara[i]!=0) mySet.insert(ara[i]);
            if(ara[i]!=0) myMulSet.insert(ara[i]);
        }
        ...
        for(int i=0; i<q; i++) {
            ...
            if(qType==1) {
                ...
                if(val!=0) myMulSet.erase(myMulSet.find(val));

                if(myMulSet.find(val)==myMulSet.end()) {
                    mySet.erase(val);
                }

                ara[p] = v;
                if(v!=0) myMulSet.insert(v);
                if(v!=0) mySet.insert(v);
            }
            else {
                cout << mySet.size() << "\n";
            }
